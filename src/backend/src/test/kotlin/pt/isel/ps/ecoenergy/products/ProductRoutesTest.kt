package pt.isel.ps.ecoenergy.products
        ...
        testApplication {
            testClient()
                .post(Uris.API + Uris.PRODUCT) {
                    ...
                    setBody(CreateProductRequest("newProduct"))
                }.also { response ->
    ...
    fun `Create Product - Product already exists`() =
        testApplication {
            testClient()
                .post(Uris.API + Uris.PRODUCT) {
                    headers.append("Authorization", "Bearer $token")
                    setBody(CreateProductRequest("Product 1"))
                }.also { response ->
        ...
        testApplication {
            testClient()
                .put(Uris.API + Uris.PRODUCT_BY_ID) {
                    headers.append("Authorization", "Bearer $token")
                    parameter("id", 2)
                    setBody(UpdateProductRequest("updatedProduct"))
                }.also { response ->
                    response.shouldHaveStatus(HttpStatusCode.OK)
                    response.shouldHaveContentType(ContentType.Application.Json)
                }
    ...
    fun `Update Product - Unauthorized`() =
        testApplication {
            testClient()
                .put(Uris.API + Uris.PRODUCT_BY_ID) {
                    parameter("id", 2)
                    setBody(UpdateProductRequest("newProduct"))
                }.also { response ->
                    response.body<Problem>().type.shouldBeEqual(Problem.unauthorized.type)
                    response.shouldHaveStatus(HttpStatusCode.Unauthorized)
                    response.shouldHaveContentType(ContentType.Application.ProblemJson)
        ...
        testApplication {
            testClient()
                .put(Uris.API + Uris.PRODUCT_BY_ID) {
                    headers.append("Authorization", "Bearer $token")
                    parameter("id", -1)
                    setBody(UpdateProductRequest("nonExistingProduct"))
                }.also { response ->
                    response.body<Problem>().type.shouldBeEqual(Problem.productNotFound.type)
                    response.shouldHaveStatus(HttpStatusCode.NotFound)
                    response.shouldHaveContentType(ContentType.Application.ProblemJson)
